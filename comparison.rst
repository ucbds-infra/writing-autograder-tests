...
A fundamental aspect of doctests is that the pass/fail behavior is determined by **string comparisons**. 
When a doctest is run, the code provided in the Python interpreter format is executed line-by-line and 
then the output of that line is expected to equal the output shown in the doctest.
...
Statements that return the Python value ``None`` have no output. If you put ``None`` into a Python 
...
Some statements that return the value ``None`` include assignment statements, import statements, 
calls to functions like ``print`` and ``exec``, and any functions with no ``return`` statement or a 
return statement that returns ``None`` itself.
...
same as their variable names: ``True`` and ``False``.
...
numeric values (e.g. ``int``, ``float``, and all of the NumPy types) and rounding errors can occur 
...
When working with integers, almost all data types that represent them have the same string 
representation. For this reason, it is a relatively easy thing to write doctests that compare integer 
...
are better-suited to writing doctests. One of the best examples is NumPy's ``isclose`` function, which 
compares two values to each other within an adjustable tolerance (which defaults to ``1e-8``). Because 
...
solution (b), the student would fail the test. Using a function like ``np.isclose``, this is avoided:
...
values. (Note that NumPy also provides ``np.allclose`` for element-wise comparison of values in 
iterables.)
...
the ``'`` character appears **in** the string, Python's default string delimeters are apostrophes. If 
...
their ``__repr__`` function returns and use that. When creating and testing custom classes, always 
use a custom ``__repr__`` function, otherwise the representation will contain the pointer to the 
...
**Always test your tests in a Python interpeter if you're unsure about the string representation of 
an object.** Don't use a Jupyter Notebook or IPython, because they don't necessary have the same 
output and they have different prompts.
